...
- (void)configSubviews {
    ...
    self.previewView.showScore = YES;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _assessmentLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        _assessmentLabel.numberOfLines = 0;
        _assessmentLabel.lineBreakMode = NSLineBreakByWordWrapping;
        [self.blurView.contentView addSubview:self.assessmentLabel];
        [self configProgressView];
        _helper = [PhotoAssessmentHelper new];
    }
    return self;
}
    ...
    if (self.isShowScore) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:asset
                                                ...
                                                resultHandler:^(UIImage *image, NSDictionary *info) {
                                                    if (image.CGImage && self.asset == asset) {
                                                        [self.helper requestMPSAssessmentScoreFor:image.CGImage completionHandler:^(PhotoAssessmentResult * _Nonnull result) {
                                                            dispatch_async(dispatch_get_main_queue(), ^{
                                                                ...
                                                                self.assessmentLabel.text = result.description;
                                                                [self resizeSubviews];
                                                            });
                                                        }];
                                                    }
...
- (void)resizeSubviews {
    ...
    [self.assessmentLabel sizeToFit];
    self.blurView.frame = CGRectMake(20, self.frame.size.height - self.assessmentLabel.frame.size.height - 44, self.assessmentLabel.frame.size.width, self.assessmentLabel.frame.size.height);
}
